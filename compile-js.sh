...
if [ "$1" = "dist" ];then
    uglifyjs ${INPUT} ${COMPRESS_OPTIONS} ${OTHER_ARGUMENTS} --source-map dist/script.js.map --output dist/script.js
else
    uglifyjs ${INPUT} --beautify --source-map dist/script.js.map --output dist/script.js
fi
